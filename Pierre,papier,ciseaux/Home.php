...
if (isset($_POST) && isset($_POST['nom'])) {
  ...
  $is_successful = $sth->execute(['nom' => $_POST['nom'], 'adresseIP' => $_POST['adresseIP']]);
  if ($is_successful)
    echo "Enregistrement reussi";
  $selectLast = $dbh->query('SELECT id FROM utilisateur ORDER BY id DESC LIMIT 1');
  $lastId = $selectLast->fetch();
  echo $lastId['id'];
  $_SESSION['user_id'] = $lastId['id']; // $lastId['id'] cette variable contient le dernier id<br>
}
